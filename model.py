...
class NodeUpdateNetwork(nn.Module):
    ...
    def forward(self, g, node_feat, edge_feat):
        ...
        e = self.e_GRU(torch.abs(g.edata['src']-g.edata['dst']), g.edata['e'])
        g.edata['e'] = e
...
class GRU_minimal(torch.nn.Module):
    def __init__(self, input_features, state_size):
        ...
        self.weights = torch.nn.Parameter(
            torch.empty(2 * state_size, input_features + state_size))
        self.bias = torch.nn.Parameter(torch.empty(2 * state_size))
    ...
    def forward(self, input, state):
        ...
        gates = gate_weights.chunk(2, dim=1)

        z_gate = torch.sigmoid(gates[0])
        # Here we use an ELU instead of the usual tanh.
        candidate_h = torch.tanh(gates[1])

        # Compute the new hidden state and output.
        new_h = old_h * z_gate + candidate_h * (1 - z_gate)
