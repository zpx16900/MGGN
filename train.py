...
class ModelTrainer(object):
    ...
    def train(self):
        ...
        for iter in range(self.global_step + 1, tt.arg.train_iteration + 1):
            ...
            query_edge_accr_layer = torch.sum(full_edge_accr_layer * query_edge_mask * evaluation_mask) / torch.sum(
            ...
            query_node_pred_layer = torch.bmm(ect[:, 0, num_supports:, :num_supports],
                ...
                for c in range(edge_feat.size(3)):
                    if edge_feat[a, 0, b, c] != 0:
                        if not 'gcx' in dir():
                            ...
                            gcx = torch.cat(
                                (gcx, torch.tensor([[a * edge_feat.size(2) + b, a * edge_feat.size(2) + c]])), 0)
                ...
                for c in range(edge_feat.size(3)):
                    if edge_feat[a, 0, b, c] != 0:
                        if not 'elcx' in dir():
                            ...
                            elcx = torch.cat((elcx, torch.tensor([[edge_label[a, 0, b, c], edge_label[a, 1, b, c]]])),
                                             0)
        return elcx
...
if __name__ == '__main__':
    ...
    tt.arg.dataset_root = 'C:\\fewshot-egnn\\datasets'
